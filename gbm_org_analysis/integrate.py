...
class GbmCellData:
    ...
    def __init__(
        ...
        for location in data_locations:
            ...
            if exp_id not in orgs_per_group_by_exp:
                orgs_per_group_by_exp[exp_id] = [
                    (group, all_orgs_as_groupname.count(group))
                    for group in set(all_orgs_as_groupname)
                ]
                ...
                for group_tuple in exp_counts:
                    for group in orgs_per_group_by_group.keys():
                        if group_tuple[0] == group:
                            orgs_per_group_by_group[group] = orgs_per_group_by_group[
                                group
                            ] + [group_tuple[1]]
        ...
        if export_data is True:
            ...
            data["cn_raw"].to_excel(cn_file, sheet_name="cn_raw", index=False)
            if normalization is True:
                data["cn_norm"].to_excel(cn_file, sheet_name="cn_norm", index=False)
            cn_file.save()

            inv_file = pd.ExcelWriter(export_data_location + "inv.xlsx")
            data["inv_raw"].to_excel(inv_file, sheet_name="inv_raw", index=False)
            if normalization is True:
                data["inv_norm"].to_excel(inv_file, sheet_name="inv_norm", index=False)
            ...
            data["dts"].to_excel(dts_file, sheet_name="dts_tidy", index=False)
        ...
        if figures in ["show", "save"]:
            ...
            if figures == "show":
                for key, value in data.items():
                    ...
                    if key == "dts":
                        ...
                        figs.bargraph(value, group_colors=colors, y_label=y_label)
            ...
            if figures == "save":
                ...
                kde = figs.norm_stacked_kde(self.dts_bygroup_tidy, group_colors=colors)
...
class OrganoidData:
    def __init__(
        ...
        if export_data is True:
            ...
            data["sa_raw"].to_excel(sa_file, sheet_name="sa_raw", index=False)
            if normalization is True:
                data["sa_norm"].to_excel(sa_file, sheet_name="sa_norm", index=False)
            sa_file.save()

            vol_file = pd.ExcelWriter(export_data_location + "vol.xlsx")
            data["vol_raw"].to_excel(vol_file, sheet_name="vol_raw", index=False)
            if normalization is True:
                data["vol_norm"].to_excel(vol_file, sheet_name="vol_norm", index=False)
            vol_file.save()
        ...
        if figures in ["show", "save"]:
            ...
            if figures == "save":
                ...
                for key, value in data.items():
                    ...
                    if key.startswith("vol"):
                        y_label = "Volume"
                    plot = figs.bargraph(value, group_colors=colors, y_label=y_label)
                    filename = export_figs_location + f"{key}_bargraph.pdf"
                    figs.figtofile(plot, filename)
